...
Steam), есть пара полей `statusDate` и `statusLine`: они рендерятся строкой
«Статус на <дата>: …» под заголовком. Обновление статуса: поправить эти две
строки и `updated`, затем `scripts/deploy.sh`. Проверка не пускает дату из
будущего и одно поле без другого.
...
`node scripts/metrika-clusters.mjs <папка с выгрузками Метрики>` печатает
сводку по дню: источники, кластеры запросов, страницы, цели.
